...
export default class Tree {
    ...
    loadItemById(item_id) {
        ...
        let from_index = visible_index - Math.floor(this.provider.constants.LIMIT_VISIBLE / 2);
    ...
    identifiersRange(from_index) {
        ...
        from_index = Math.min(from_index, this.indexes.visible.length - this.provider.constants.LIMIT_VISIBLE);
        ...
        this.indexes.visible.forEach((item, i) => {
            ...
            if (identifiers.length < this.provider.constants.LIMIT_VISIBLE) {
    ...
    updateLoadButtons() {
        ...
        this.load_buttons = {
            up: {
                ...
                visible: this.indexes.visible.length - this.provider.constants.LIMIT_VISIBLE > 0
            },
            down: {
                disabled: this.is_loading,
                visible: this.last_from_index < this.indexes.visible.length - this.provider.constants.LIMIT_VISIBLE - 1
            }
        };
    }
